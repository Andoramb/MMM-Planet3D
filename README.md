...
| `clouds.source`          | string | `"static"` | `"static"` (vendored Blue Marble clouds, no network) \| `"dynamic"` (same vendored texture, but animated - see [Clouds](#daynight-and-clouds) below) \| `"realtime"` (fetched from NASA GIBS, polled every 24h). Only used when `enabled` is `true`. |
...
- `source: "dynamic"` uses the same offline Blue Marble texture as `"static"` (no network), but renders it less like a rigid decal: a second, fainter sphere is added a bit further out, counter-rotating relative to the base layer, and both layers' UVs are scrolled and lightly warped by a domain-warped noise field in the fragment shader (`onBeforeCompile` again, same technique as the night-mask patch below) so the cloud pattern visibly drifts and billows over time instead of just spinning in place. All tuning constants (scroll speed, warp strength/scale, high-layer altitude/opacity/rotation) live at the top of `public/CloudsLayer.mjs` next to the ones above.
